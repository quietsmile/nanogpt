"""Decompose block 1 MoE diff using ref dumps."""
...
# ref block 0 output serves as ref block 1 input
...
ref_self_attn = load_out(f"{DUMP}/decoder.layers.1.self_attention-iter5988-mbs0-forward-output-tp0.1-pp0.1-ep3.4.pt")
...
# Convert ref_b0 to nano format
x = ref_b0.transpose(0, 1).contiguous()
B, T, H = x.shape
...
with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.bfloat16):
    # Run nano block 1 fully first
    out_nano, _ = b1(x)
    diff(out_nano.transpose(0, 1), ref_b1, "block 1 nano baseline")

    # Run nano attn on ref input
    a1 = b1.attn
    h_ln = b1.ln_1(x)
    ...
    q, k = a1.rotary_emb(q, k, seq_len=T)
    ...
    mlp = b1.mlp
    shared_out = mlp.shared_expert(mlp_in.view(-1, H))
    diff(shared_out.view(B, T, H).transpose(0, 1), ref_shared, "shared_expert output")

    # Full MoE output
    mlp_out, _ = mlp(mlp_in)
    diff(mlp_out.transpose(0, 1), ref_mlp, "MoE mlp output")

    # Feed ref's mlp input, check nano MoE output
    ref_mlp_in_tuple = torch.load(f"{DUMP}/decoder.layers.1.mlp-iter5988-mbs0-forward-input-tp0.1-pp0.1-ep3.4.pt", weights_only=False, map_location="cuda")
    ref_mlp_in = ref_mlp_in_tuple[0]
    ref_mlp_in_bth = ref_mlp_in.transpose(0, 1).contiguous()
    mlp_out_ref_in, _ = mlp(ref_mlp_in_bth)
    diff(mlp_out_ref_in.transpose(0, 1), ref_mlp, "MoE on ref's exact input")
