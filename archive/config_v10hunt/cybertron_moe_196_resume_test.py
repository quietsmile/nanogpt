"""Short 500-iter run to verify the aux-free bias fix closes the 1+ nat gap.

Identical to cybertron_moe_196_resume.py except:
- max_iters=500 (quick turnaround)
- out_dir is separate so we don't clobber the full-run artefacts
- eval_interval bumped so we don't waste time on val mid-run
"""
out_dir = 'out-cybertron-moe-196-resume-biasfix'
eval_interval = 250
log_interval = 1
eval_iters = 10
eval_only = False
...
max_iters = 500  # short run for bias-fix verification
